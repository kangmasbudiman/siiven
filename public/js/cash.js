jQuery(function ($) {
	...
	const edit = ({idCash, tanggal, total}) => {
		const modal = $('#edit')
		const form = modal.find('form')[0]
		const url = updateUrl.replace(':id', idCash)

		reset(form)
		form.action = url

		modal.find('[name=id]').val(idCash)
		modal.find('[name=tanggal]').val(tanggal)
		modal.find('[name=total]').val(total)
	...
	const remove = id => {
		if (confirm('Hapus data Cash ini?')) {
